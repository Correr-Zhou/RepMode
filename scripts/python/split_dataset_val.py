...
def main():
    ...
    name = opts.src_csv.split('/')[-2]
    path_store_split = os.path.join(opts.dst_dir, name)
    path_train_csv = os.path.join(path_store_split, 'train.csv')
    path_val_csv = os.path.join(path_store_split, 'val.csv')
    if os.path.exists(path_train_csv) and os.path.exists(path_val_csv):
    ...
    df_val = df_all[idx_split:]
    vprint('train/val sizes: {:d}/{:d}'.format(len(df_train), len(df_val)))
    if not os.path.exists(path_store_split):
        os.makedirs(path_store_split)
    df_train.to_csv(path_train_csv, index=False)
    df_val.to_csv(path_val_csv, index=False)
    vprint('saved:', path_train_csv)
    vprint('saved:', path_val_csv)
